...
<div class="grid place-items-center grid-cols-1 gap-10 xl:grid-cols-2 lg:grid-cols-2 mt-11 2xl:grid-cols-3"> 
    ...
    <script>
        ...
        document.querySelector('.leading-5').classList("none")
    const search_keyword = document.location.search
    const str_search_replace = search_keyword.replace('?keyword=', '')
    if(search_keyword = '') { }
        document.querySelector('.keyword').value = search_keyword.replace('')
    </script>
@endsection
